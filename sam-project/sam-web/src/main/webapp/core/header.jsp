...
<nav>
    ...
    <%} if (ControleAutorizacao.checkPermissao("transacoes", usuario.getTipo()) && !ControleAutorizacao.checkBloqueio("transacoes", usuario.getCPF())) {%>
    ...
    <a href="/sam/core/geral/inicio.jsp">Início</a>
    <% if (ControleAutorizacao.checkPermissao("notificacoes", usuario.getTipo()) && !ControleAutorizacao.checkBloqueio("notificacoes", usuario.getCPF())) {%>
    ...
    <div id="hamburger-dropdown" class="dropdown-content">
        ...
        <%} if (ControleAutorizacao.checkPermissao("transacoes", usuario.getTipo()) && !ControleAutorizacao.checkBloqueio("transacoes", usuario.getCPF())) {
                if(usuario.getTipo() == UsuarioTipo.CLIENTE){
                    if(usuario.getIdGestor() != null){%>
        <a href="/sam/core/geral/transacoes.jsp">Transações</a>
        <%}} else {%>
        <a href="/sam/core/geral/transacoes.jsp">Transações</a>
        <%}} if (ControleAutorizacao.checkPermissao("empresas", usuario.getTipo()) && !ControleAutorizacao.checkBloqueio("empresas", usuario.getCPF())) {%>
        ...
        <a href="<%=request.getContextPath()%>/core/gestor/computar-milhas.jsp">Computar Milhas</a>
        ...
        <a href="<%=request.getContextPath()%>/core/gestor/orcamentos.jsp">Orçamentos</a>
        <%} if (ControleAutorizacao.checkPermissao("planos", usuario.getTipo()) && !ControleAutorizacao.checkBloqueio("planos", usuario.getCPF())) {%>
        <a href="<%=request.getContextPath()%>/core/gestor/planos.jsp">Planos</a>
