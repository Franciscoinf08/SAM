...
<main>
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <a href="<%= contextPath %>/core/cliente/criarOuAlterarFormulario.jsp?id=<%= form.getId() %>">
                    <button>Editar</button>
                </a>

                <form action="<%= contextPath %>/excluirFormulario" method="POST" style="display: inline;">
                    <input type="hidden" name="id" value="<%= form.getId() %>" />
                    <button type="submit" style="background-color: #cc0000;"
                            onclick="return confirm('Tem certeza que deseja excluir o formulário <%= form.getId() %>?');">
                        Excluir
                    </button>
                </form>
            </td>
